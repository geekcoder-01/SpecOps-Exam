...
def get_library_subjects(
    ...
    records = (
        ...
        .filter(QuestionLibrarySubject.bank_id == bank_id)
        .group_by(QuestionLibrarySubject.library_subject_id)
        .order_by(QuestionLibrarySubject.subject_name.asc())
        .all()
    )
...
def update_library_subject(
    ...
    if not subject:
        raise HTTPException(status_code=404, detail="Library subject not found")
    ...
    duplicate = (
        db.query(QuestionLibrarySubject)
        .filter(
            QuestionLibrarySubject.bank_id == subject.bank_id,
            func.lower(QuestionLibrarySubject.subject_name)
            == new_name.lower(),
            QuestionLibrarySubject.library_subject_id
            != library_subject_id,
...
def delete_library_subject(
    ...
    question_count = (
        ...
        .filter(Question.library_subject_id == library_subject_id)
        .scalar()
    )
    ...
    if question_count > 0:
        raise HTTPException(
            status_code=400,
            detail="Move or delete the questions before deleting this subject",
        )

    db.delete(subject)
    db.commit()

    return {"message": "Library subject deleted successfully"}